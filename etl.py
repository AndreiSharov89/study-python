# создаёт из state_abbr.csv и Export.csv дополнительные csv, которые копируются в БД
...
for market in market_list:
    ...
    for category in category_names_list:
        if market[category].lower() == 'y':
            markets_categories_list.append(
                {'market_category_id': ctr_markets_categories, 'market_id': market['FMID'],
...
# print(states_list)
def create_csv():
    with open(STATES_OUTPUT, 'w', newline='', encoding='utf8') as file:
    ...
    print(cities_list)
    with open(CITIES_OUTPUT, 'w', newline='', encoding='utf8') as file:
    ...
    # print(markets_list)
    with open(MARKETS_OUTPUT, 'w', newline='', encoding='utf8') as file:
    ...
    with open(CATGEORIES_OUTPUT, 'w', newline='', encoding='utf8') as file:
        writer = csv.DictWriter(file, fieldnames=categories_list[0].keys())
        writer.writeheader()
        for row in categories_list:
            writer.writerow(row)

    with open(MARKETS_CATGEORIES_OUTPUT, 'w', newline='', encoding='utf8') as file:
